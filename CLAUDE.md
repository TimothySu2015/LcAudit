...
被盜帳號的玩家多半直接雙擊執行檔。**雙擊路徑必須自己能走完**，不能假設使用者會開命令提示字元帶參數。

`ConsoleLaunch.IsOwnConsole()` 以 `GetConsoleProcessList` 判斷啟動方式 —— 雙擊時 Windows 建立新主控台、只有本行程附著（count == 1）；從 cmd／PowerShell 執行時那個 shell 也附著在同一個主控台上。搭配 `AuditOptions.IsDefaultRun`（沒帶任何影響行為的參數）決定是否走 `InteractiveWizard`。

引導流程會主動處理三件使用者不會自己做的事：**提權**（`ConsoleLaunch.TryRelaunchElevated`，UAC 被取消是正常選擇不是錯誤）、**問事發時間**、**問要不要上傳**，最後**停住等按鍵**——否則雙擊時視窗會直接關掉，使用者什麼都看不到。

**帶任何參數就不走引導** —— 有打參數代表使用者知道自己在做什麼，不該再用問答打斷他，也才不會破壞腳本化使用。

## 事發時間是最有價值的一個輸入（`--incident-time` / `--incident-end`）
...
**錨點是區間而非時間點**（`IncidentWindow`）。受害者實際給得出的是「02:00 我還在線上掛機，07:30 發現東西不見了」—— 中間的「大約 04:00」只是推估。若只吃單一時間點，靠近另一端的事件（可能正是真正動手的時刻）反而不會被凸顯。落在區間內的跡證距離為零，排最前面並標示「★ 就在事發區間內」。

**刻意不設距離上限。** 入侵的原因往往遠早於受害者「發現」的時間 —— 攻擊者可能數週前就植入了遠端工具，潛伏到適當時機才動手。硬性濾掉超過 N 天的事件，等於替使用者做了「這無關」的判斷，而最關鍵的證據可能就在那裡。改為全部依距離排序取前 50 筆，遠的自然沉底並標明「事發前 30 天」。
