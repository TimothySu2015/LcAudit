...
public sealed class HtmlReporter
{
    ...
    private static void AppendIncidentTimeline(StringBuilder sb, AuditReport report)
    {
        ...
        sb.AppendLine($"<h2>事發時間比對（{Esc(window.Describe())}）</h2>");

        if (matches.Count == 0)
        {
            sb.AppendLine("""
                <p class="empty">沒有找到任何帶時間點的跡證可供比對。
                相關紀錄可能已被清除，或超過事件記錄的保留期。</p>
                """);
            return;
        }

        var within = matches.Count(m => m.IsWithinWindow);

        if (closest.Count > 0)
        {
            sb.AppendLine("<div class=\"card notice\"><strong>"
                          + (within > 0
                              ? $"有 {within} 筆跡證就發生在事發區間內"
                              : $"有 {closest.Count} 筆跡證與事發時間相近（2 小時內）")
                          + "</strong> —— 這些最值得優先查證。</div>");
        }
    ...
    private static void AppendForensicNotice(StringBuilder sb, AuditReport report)
    {
        sb.AppendLine("""
            ...
            <h2>關於這份報告的隱私</h2>
            <div class="card">
            <p><strong>這份報告只存在你自己的電腦上。</strong>本工具不會自動傳送任何資料。</p>
            <p>報告寄出功能<strong>預設關閉</strong>。只有在你需要作者協助判讀時，
            才需要主動選擇送出 —— 不送出完全不影響檢查結果。</p>
            <p>若你選擇送出，報告內容<strong>僅用於分析你這台電腦的資安弱點與可能的入侵途徑</strong>，
            不作其他用途。</p>
            <p class="meta">報告中含電腦名稱、使用者帳號、曾遠端連入的來源 IP、已安裝程式路徑，
            但不含密碼、金鑰或遊戲帳號內容。轉交給任何人之前，請自己先看過一遍。</p>
            </div>
            """);
